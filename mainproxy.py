...
def get_live_proxy(retries=10, backoff_factor=1.5):
    check_urls = [
        "http://httpbin.org/ip",
            ...
            for url in check_urls:
                try:
                    ...
                    if response.status_code == 200:
                        try:
                            ip = response.json().get("origin") or response.json().get("ip") or "Unknown"
                        except Exception:
                            ip = response.text.strip()
                        print_info("Proxy Status", f"Active proxy: {ip}", Fore.GREEN)
                        return proxies_dict
                except Exception:
                    continue
        wait = backoff_factor ** attempt
        print_info("Proxy Retry", f"Retry ({attempt}/{retries}) failed... trying again in {wait:.1f} seconds...", Fore.YELLOW)
        time.sleep(wait)

    raise Exception("No active proxy found after all attempts.")

# API Functions
def make_request(method, url, headers, data=None, retries=MAX_RETRIES, proxies=None):
    for attempt in range(retries):
        try:
            response = requests.request(method, url, headers=headers, json=data, proxies=proxies, timeout=10)
...
def get_token_info(token, proxies):
...
def check_is_claimable(token, proxies):
...
def claim_airdrop(token, proxies):
    url = f"{BASE_URL}/token/claim-airdrop"
    headers = MAIN_HEADERS.copy()
    headers['authorization'] = f'Bearer {token}'
    response = make_request("POST", url, headers, proxies=proxies)
    if response and response.get('statusCode') == 200:
...
def login_email():
    ...
    try:
        ...
        proxies = get_live_proxy()

        response = check_passcode(login_id, passcode, proxies)
        ...
        response = send_otp_email(email, login_id, passcode, proxies)
        if not response or response.get("statusCode") != 200:
        ...
        response = check_otp_email(login_id, otp, proxies)
        if response and response.get("statusCode") == 200:
        ...
        print_info("Proxy Error", str(e), Fore.RED)
...
def login_telegram():
    ...
    try:
        ...
        response = send_otp_telegram(login_id, passcode, chat_id, proxies)
        if not response or response.get("statusCode") != 200:
        ...
        response = verify_otp_telegram(login_id, otp, proxies)
        if response and response.get("statusCode") == 200:
            jwt_token = response["data"]["jwtToken"]
            print_info("JWT Token", jwt_token, Fore.GREEN)
            save_jwt_token(jwt_token)
...
def process_daily_claim():
    ...
    for idx, token in enumerate(tokens, start=1):
        try:
            ...
            proxies = get_live_proxy()
            login_id = get_current_user(token, proxies)
            if login_id is None:
                continue
            get_token_info(token, proxies)
            if check_is_claimable(token, proxies):
                claim_airdrop(token, proxies)
        except Exception as e:
            print_info("Proxy Error", f"Failed to process account {idx}: {str(e)}", Fore.RED)
            continue
